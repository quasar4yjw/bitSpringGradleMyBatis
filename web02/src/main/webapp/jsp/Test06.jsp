...
=> ServletRequest(request), HttpSession(session), ServletContext(application)
, PageContext(page) 로부터
객체 꺼내기 또는 객체 생성

속성
id => 레퍼런스 변수
scope => 객체를 꺼낼 보관소 지정. page, request, session, application
class => 보관소에서 꺼낼 객체의 타입, 또는 객체 생성을 위한 타입을 지정.
type => 레퍼런스 변수의 타입
...
java util.List products = 
  (java.util.List)request.getAttribute("products");
if (products == null) {
